...
NodeDistribution::NodeDistribution(double min, double max){
	vector<double> pr(2);
	int answer = 0;
...
NodeDistribution::NodeDistribution(Node* root){
	this->root = root;
	this->lastParent = root;
	vector<double> pr(2);
	int answer = 0;
	pr[answer]= 1.0; pr[1-answer] = 1 - pr[answer];//the chose interval was [min, max], with cr=100%
//	zoomInto(root, pr, 0);
}
void NodeDistribution::readLeafs(Node* node){//BUGGED
...
int NodeDistribution::readLeafs(Node*& node, string& xfx, int & nPairs, vector<double>& x, vector<double>& Fx, double parentProb){//BUG
	if(node->getChildrenCount()==0){
		//double parCred = 1;
		//if(node->getParentsCount()>0){
		//	Node* parent = node->getParent(0);
		...
		sprintf_s(sLower, "%.100f", node->lower);
		sprintf_s(sUpper, "%.100f", node->upper);
		double _fx = nodeCred /(node->upper - node->lower);
		char sf[4000];
		sprintf_s(sf, "%.100f", _fx);
		//std::ostringstream oss_fx; oss_fx << _fx; string trash_sf = oss_min.str();
...
void NodeDistribution::zoomInto(Node* parent, vector<double> pr, int answer){
