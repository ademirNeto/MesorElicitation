...
*/
/*
          OBTAIN THE location FACTOR EXP(-MU) AND
...
void cdfgam(int *which,double *p,double *q,double *x,double *scaling,
...
/**********************************************************************
     ...
     WHICH --> Integer indicating which of the next four argument
               values is to be calculated from the others.
               Legal range: 1..4
               iwhich = 1 : Calculate P and Q from X,scaling and SCALE
               iwhich = 2 : Calculate X from P,Q,scaling and SCALE
               iwhich = 3 : Calculate scaling from P,Q,X and SCALE
               iwhich = 4 : Calculate SCALE from P,Q,X and scaling
     ...
     scaling <--> The scaling parameter of the gamma density.
                Input range: (0, +infinity).
                Search range: [1E-300,1E300]
     ...
     STATUS <-- 0 if calculation completed correctly
               -I if input parameter number I is out of range
                ...
                10 if the gamma or inverse gamma routine cannot
                   ...
                   X and scaling very large (gt 1E10 or more)
...
*/
        ...
        *status = 0;
        dinvr(status,scaling,&fx,&qleft,&qhi);
S250:
        if(!(*status == 1)) goto S290;
        cumgam(&xscale,scaling,&cum,&ccum);
        if(!qporq) goto S260;
        fx = cum-*p;
        goto S270;
S260:
...
**********************************************************************
     ...
     A --> The scaling parameter of the incomplete gamma.
                                                A is DOUBLE PRECISION
